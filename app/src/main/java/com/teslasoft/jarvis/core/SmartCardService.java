...
public class SmartCardService extends Service {
	...
	public void start(Context context) {
		if (isMyServiceRunning(com.teslasoft.jarvis.core.AutoRunService.class)) {
			try {
				stopService(new Intent(com.teslasoft.jarvis.core.SmartCardService.this, com.teslasoft.jarvis.core.AutoRunService.class));
			} catch (Exception ignored) {}
		}

		// Android 6/7/8 is no longer supported
		int notifyID = 2;
		String CHANNEL_ID = "SmartCardServer";
		CharSequence name = "SmartCard";
		...
		PendingIntent serv = PendingIntent.getActivity(this, 0, alls, PendingIntent.FLAG_MUTABLE);
		PendingIntent pendingIntent = PendingIntent.getActivity(this, 0, notificationIntent, PendingIntent.FLAG_MUTABLE);
		Notification notification = new Notification.Builder(context)
			.setContentTitle("Debug")
			.setVisibility(Notification.VISIBILITY_PUBLIC)
			.setContentText("SmartCard server is running...")
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
		try {
			start(com.teslasoft.jarvis.core.SmartCardService.this);
		} catch (Exception ignored) {}
		...
		handler.postDelayed(() -> {
			...
			timer.scheduleAtFixedRate(new TimerTask() {
					...
					public void run() {
						...
						if (pm.getComponentEnabledSetting(new ComponentName(SmartCardService.this, DataProtectorService.class)) == PackageManager.COMPONENT_ENABLED_STATE_ENABLED) {
							if (!isMyServiceRunning(DataProtectorService.class)) {
								try {
									startService(new Intent(SmartCardService.this, DataProtectorService.class));
								} catch (Exception ignored) {}
							}
						}
					}
